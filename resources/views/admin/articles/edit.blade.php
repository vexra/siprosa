<x-app-layout>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.articles.update', $article) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $article->slug) }}" class="mt-1 block w-full rounded-md shadow-sm border-gray-300 bg-gray-100" readonly>
                            @error('slug')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="description" id="description" rows="10" class="mt-1 block w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>{{ old('description', $article->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        ...
        titleInput.addEventListener('input', function() {
            ...
            // Menghilangkan karakter khusus dan menggantinya dengan tanda hubung (-)
